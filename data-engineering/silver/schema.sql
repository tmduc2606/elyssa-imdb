-- Silver Layer DDL — 14-table 3NF/BCNF schema per Duke's schema_draft.sql
-- Includes SCD2 tracking on dim_title and dim_name, surrogate key sequences,
-- TimescaleDB hypertable for rating snapshots, and quality/governance tables.
...
COMMENT ON COLUMN silver.title_basics.end_year IS 'End year for TV series, NULL otherwise';
...
COMMENT ON TABLE silver.title_episode IS 'Links an episode to its parent TV series';
...
COMMENT ON TABLE silver.title_akas IS 'Alternative titles for a title, including original and localised versions';
...
COMMENT ON TABLE silver.title_akas_type IS 'One row per alternative title type';
...
COMMENT ON TABLE silver.title_akas_attribute IS 'Additional free-form descriptors for an alternative title';
...
COMMENT ON TABLE silver.title_principal IS 'Principal cast/crew for a title';
COMMENT ON COLUMN silver.title_principal.job IS 'Specific job (e.g., "director of photography"), NULL for actors';
...
COMMENT ON TABLE silver.title_principal_char IS 'Characters played by a principal';
COMMENT ON COLUMN silver.title_principal_char.character_name IS 'Assumes character names are unique within a principal record';
...
COMMENT ON TABLE silver.name_profession IS 'Top 3 professions of a person, in order of importance';
...
COMMENT ON TABLE silver.name_known_for_title IS 'Titles a person is known for, in order of prominence';
...
COMMENT ON TABLE silver.graph_sync_status IS 'Tracks Neo4j sync completion timestamps';
...
COMMENT ON TABLE silver.data_quality_log IS 'Stores daily data quality check results';
